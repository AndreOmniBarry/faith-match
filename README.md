...
its SDK. A full live-service match-3 foundation — daily engagement, a real
special-tile/combo system, escalating difficulty, lives, and a reward economy
— with its own modes, its own "chaos," and a content pipeline meant to keep
growing for years without hand-authoring every level forever.
...
- **Match 3** → pop. **Match 4** → Striped (clears the row/column). **L/T
  ...
  tile of one color).
- **Combine two specials** by swapping them into each other for the full
  special-combo pairing table — the biggest chaos on the board.
...
typical mobile match-3 ramp for standard levels, genuinely harder for
finales) — see `server/app/level_gen.py` for the authoritative version, which
...
target, finale modifiers) is deterministic per level number, the same way a
level's design is fixed in most match-3 games while the board itself isn't.
No amount of replaying makes a level memorizable.
